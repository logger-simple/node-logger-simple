...
class Logger extends EventEmitter {
    ...
    try {
      ...
      this.testConnection()
        .then(() => {
          ...
          this.emit('ready');
          this.emit('connected');
          this._log('[Logger] Ready and connected');
        })
        .catch((error) => {
          this.emit('error', error);
          this._error('[Logger] Connection failed:', error.message);
        });

    } catch (error) {
      this.emit('error', error);
      this._error('[Logger] Initialization failed:', error.message);
    }
  }
  ...
  async sendLog(logLevel, message, context = null) {
    if (this.isShuttingDown) {
      this._warn('[Logger] Skipping log during shutdown');
      return null;
    }
    ...
    try {
      ...
      this._error(`[Logger] Failed to send ${logLevel} log:`, error.message);
      throw error;
    }
  }
  ...
  async sendHeartbeat() {
    try {
      ...
      this._error('[Logger] Heartbeat failed:', error.message);
      throw error;
    }
  }
  ...
  startHeartbeat() {
    ...
    this._log('[Logger] Heartbeat started');
  }
  ...
  stopHeartbeat() {
    if (this.heartbeatTimer) {
      ...
      this.heartbeatTimer = null;
      this._log('[Logger] Heartbeat stopped');
    }
  }
  ...
  async flushLogs() {
    ...
    try {
      ...
      this._error('[Logger] Batch processing failed:', error.message);
    }
  }
  ...
  setupCrashDetection() {
    ...
    process.on('uncaughtException', async (error) => {
      this._error('[Logger] Uncaught Exception:', error.message);
      ...
      try {
        ...
        this._error('[Logger] Failed to log uncaught exception');
      }
    ...
    process.on('unhandledRejection', async (reason) => {
      this._error('[Logger] Unhandled Promise Rejection:', reason);
      ...
      try {
        ...
        this._error('[Logger] Failed to log unhandled rejection');
      }
    });
  }
  ...
  setupGracefulShutdown() {
    ...
    signals.forEach(signal => {
      process.on(signal, async () => {
        ...
        this._log(`[Logger] Received ${signal}, shutting down gracefully...`);
        await this.shutdown();
        process.exit(0);
      });
    });
    ...
    for (let attempt = 1; attempt <= this.options.retryAttempts; attempt++) {
      try {
        ...
        if (attempt < this.options.retryAttempts) {
          ...
          this._warn(`[Logger] Request failed (attempt ${attempt}), retrying in ${delay}ms...`);
  ...
  async shutdown() {
    ...
    this._log('[Logger] Starting graceful shutdown...');
    ...
    try {
      ...
      this._log('[Logger] Graceful shutdown completed');

    } catch (error) {
      this.emit('shutdown', { graceful: false, error });
      this._error('[Logger] Error during shutdown:', error.message);
    }
  }
